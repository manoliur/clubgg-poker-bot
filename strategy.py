...
Постфлоп-решение собрано в явную матрицу (см. «матрица постфлопа» ниже):
    решение = f(сила руки, опасность доски, активность оппонента, игроков, улица)
Все пять величин считает Spot по одному разу за решение, а выбирают клетку две
функции — _answer_bet (перед нами ставка) и _lead (нам чекнули / мы первые).
...
# А если поднимали МЫ, то ставка поверх видна без запаса по размеру: доплата
# от одного открытия — уже ререйз, в том числе мин-рейз в 3ББ (в хедз-апе
# 3-бетят именно так). Живая раздача #4 от 22.08: наш рейз 2.5 -> его 3-бет 3.0 не
# добирал до 1.6 открытий, бот читал его как «крупное открытие» и 3-бетил AQo
# на велью — до 5-бета и банка в 35ББ против диапазона QQ+/AK.
HERO_RERAISE_OPEN_MULT = 1.0
...
def decide_preflop(hole, position, players, has_bet, to_call_bb, pot_bb, stack_bb, chart=None,
    ...
    # ход: если мы уже поднимали, любая доплата дороже нашего открытия — это
    # ставка ПОВЕРХ нашего рейза, каким бы маленьким ни был ререйз (мин-рейз до
    # 3ББ в хедз-апе — обычный 3-бет). Второй — деньги под ставкой (is_squeeze):
    ...
    if hero_raised:
        # После НАШЕГО рейза ставка перед нами — ререйз при любом размере. Порог
        # в одно открытие оставлен только на случай, когда числа в кадре
        # прочитаны с ошибкой, и он нестрогий: мин-рейз (наши 2.5 -> его 5.0)
        # оставляет доплату ровно в открытие, а это самый частый 3-бет в HU.
        is_3bet = to_call_bb is not None and to_call_bb >= open_bb * HERO_RERAISE_OPEN_MULT
    else:
        over_mult = (RERAISE_OPEN_MULT
                     if is_squeeze(pot_bb, to_call_bb, position, live_players) else mult)
        is_3bet = to_call_bb is not None and to_call_bb > open_bb * over_mult
